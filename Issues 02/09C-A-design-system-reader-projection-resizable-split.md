...
Typography 以全宽 Type Atlas 验证 Reader Projection（见下方 2026-07-30 follow-up
与 2026-08-03 分栏退役）。其余 leaf 统一为全宽单栏页面。

这是一条完整的 Browser 纵向切片：DB-backed API 输入、Reader Projection、页面
...
（2026-08-03：全部 leaf 已改为全宽单栏，见下方退役决策。）
...
- [x] Browser 内所有状态标记统一为 4px 圆角且无描边的轻量填充矩形。
...
- [x] 自动测试覆盖 projection 信息无损、复杂 object 不泄露 JSON；e2e 守卫确认
      `ds-leaf-split` / 空样本右栏不再出现（见 2026-08-03）。
...
- [ ] Agent 打开 DB-backed Browser，确认 Typography Type Atlas 的可读投影与
      specimen 使用同一组 source facts，Technical details / Evidence 能反查原始
      entry。
...
（e2e `tests/design-system-reader.spec.ts` 已走通等价链路：alignment →
09B-rich sources 经 MCP ingest → Reader Projection 渲染 → Typography / Layout /
Color / Interaction 全宽 leaf；分栏相关断言已退役。真实 Agent 验证留待人工执行。）

## Open gaps

- ~~本地比例偏好的精确 storage lifetime 和跨项目共享范围需在实现前锁定~~
  → 已锁定后又于 2026-08-03 整体退役（见下方决策）。
- ~~自主设计的视觉结果（sheet chrome、divider 样式、stacked 布局）待设计师核对。~~
  → divider / stacked 已退役；sheet chrome 与全宽 leaf 视觉仍待设计师核对。
...
- sidebar 增加响应式 bar：760px 以下 tabs 固定 190px 可滚动——这是 stacked
  单列在窄屏真正可用的前提。（2026-08-03：stacked 分栏已退役；窄屏 sidebar bar
  仍保留。）
...
- 键盘隔离重构：旧 window capture 监听会吞掉 sheet 内键盘事件；Esc 分层由
  `sheetEscapeAction` 单一决策，Radix `react-dismissable-layer` 在 document
  capture 阶段抢先 dismiss 的问题用 `onEscapeKeyDown` preventDefault 挡掉
  （popover 内 Esc 只关 popover）。（divider 键盘调整动机已随分栏退役失效，
  隔离边界本身仍保留。）
- ~~左栏 role summary 现携带 `tracking X` / transform 段（`formatTextStyleSummary`），
  右栏 specimen annotation 复用同一 summary，不再重复打印。~~
  → Type Atlas 就地呈现 summary；不再有左右栏分工。
- principle 未知结构键不再 JSON.stringify 泄漏到主阅读层：rich shape 之外的键
  进入 `extraFields`（label + 格式化文本）；无任何已知键的对象整体走字段行投影。
- ~~不用 shadcn resizable：…（理由同时记录在 `ds-split-pane.tsx` 头注释…）。~~
- ~~共享抽取：`atomicWriteJson` … browser preferences 共用；默认比例常量单一来源在
  `design-system-browser-preferences-shared.ts`。~~
- code review 双轴已执行（历史记录保留；分栏相关修复项随 2026-08-03 删除）。
...
- `npx vitest run`：91 文件 832 测试全绿（含 Atlas、multi-hop alias、
  ...
  model 单测与 JSON 无损断言）。
- `npm run check`（typecheck + e2e）：80 passed，含当时的
...
## 设计决策 2026-08-03（分栏退役）

设计师（issue 作者）确认：**整体退役** Design System Browser 的左右分栏
（`LeafSplit`），含 Color / Materials / Component detail。理由：复杂分栏不适合
Agent 内嵌浏览器；右栏长期只有 `VisualSamplesEmpty` 占位，无真实内容。

本决策 **supersede** 上文仍有效的双栏 / 42–58 比例 / 偏好持久化 / divider 契约 /
stacked 回退等 locked decisions，以及已划掉的相关验收项。

删除物：

- `components/workbench/ds-split-pane.tsx`、`ds-split-pane-model.ts`
- `app/api/design-system-browser-preferences/`、
  `lib/runtime/design-system-browser-preferences*.ts`、
  `lib/runtime/commands/design-system-browser-preferences.ts`、
  `getDesignSystemBrowserPreferencesPath`
- `VisualSamplesEmpty`、`useDesignSystemSplitRatio`、`renderMain` 的 `layout`
  字段与 `.dsb-leaf` / split / samples CSS
- unit：`tests/unit/ds-split-pane-model.test.ts`、
  `tests/unit/design-system-browser-preferences.test.ts`
- e2e 重命名：`tests/design-system-reader-split.spec.ts` →
  `tests/design-system-reader.spec.ts`；去掉拖拽 / 键盘 / 双击 / 偏好持久化 /
  stacked 断言，保留 `ds-leaf-split` / `ds-samples-empty` count 0 退役守卫

现行契约：所有叶子页（含 Color / Materials / Component）为全宽单栏，chrome
对齐 Home / Typography / Layout / Interaction（`.dsb-page`）。未来视觉模块按
全宽页面规划，不再恢复右栏。
