...
- [ ] 项目相位非 `ready_for_new_design` 时,新设计 run 声明被 Runtime 拒绝。
- [ ] Runtime 交付给 Agent 的任务上下文仅包含 intent 与 design-system source(version 明确),不包含反馈记录、事件日志、历史 annotation。上下文契约标明 Formalized / Candidate 两级优先级(Formalized 硬参考,Candidate 软参考,冲突时 Formalized 优先且冲突显式标记)。
- [ ] Agent 生成中若实际依赖某条 Candidate 条目,须在 artifact 声明或提案中显式标注所依赖的 Candidate id;依赖记录进入事件日志,供 Issue 29 审查时按使用频率优先裁决。
- [ ] Agent 创建或更新 prototype code,并声明 source artifacts。
- [ ] Runtime 创建 prototype run record,链接到 design-system version。
- [ ] Workbench 显示新的 Prototype Evidence Surface(多嵌单活,见 Issue 30)。
- [ ] 测试覆盖 intent submit、相位门拒绝、生成上下文 payload 隔离、artifact declaration、run metadata linkage。
