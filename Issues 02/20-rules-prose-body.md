...
- [ ] schema 接受散文形态的 rule body;`meaning` 保持必填,作为唯一标题来源。
- [ ] Browser 对散文正文逐字渲染(标题 + 正文),无键名剥离、无字段拼接。
- [ ] 正文编辑端到端可用(textarea → edit-entry → 源文件 + DB + SSE 刷新)。
- [ ] 存量富对象 body 兼容:ingest 不拒绝,显示降级为可读文本,不出现裸 JSON。
- [ ] 测试:schema 单测(散文接受 / 富对象兼容 / meaning 必填)、渲染单测、e2e(散文规则从声明到显示到编辑)。
