...
test.describe("legacy listPendingSeedEvidence reader (historical compatibility)", () => {
  ...
  test("URL-only declaration resolves to seed and clears pending when screenshot present", () => {
    withTempProject((dir) => {
      const seed = registerSeedReference(dir, {
        figmaSeedReference: VALID_FIGMA,
        originalDesignIntent: INTENT
  ...
  test("pending checks only current surface screenshot — superseded shot does not clear", () => {
    withTempProject((dir) => {
      const seed = registerSeedReference(dir, {
        figmaSeedReference: VALID_FIGMA,
        originalDesignIntent: INTENT
      ...
      const withShot = recordEvidencePackage(dir, {
        seedReferenceId: seed.record.id,
        frame: { nodeId: "1:2", name: "WithShot" },
        evidenceViews: { rawData: "available", screenshot: "available" },
        screenshot: { artifactPath: "artifacts/old.png" }
      });
      expect(withShot.ok).toBe(true);
      if (!withShot.ok) return;
      expect(listPendingSeedEvidence(dir)).toEqual([]);

      const missingCurrent = recordEvidencePackage(dir, {
        seedReferenceId: seed.record.id,
        frame: { nodeId: "1:2", name: "Missing" },
        evidenceViews: { rawData: "available", screenshot: "missing" }
      });
      expect(missingCurrent.ok).toBe(true);
      if (!missingCurrent.ok) return;

      const pending = listPendingSeedEvidence(dir);
      expect(pending.map((r) => r.id)).toEqual([seed.record.id]);
    });
  });
});
