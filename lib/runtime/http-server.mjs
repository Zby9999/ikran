...
export function composeWorkbenchUrl(host, port, token) {
  // `view=workbench` persists the Seed Evidence canvas across Agent reopen /
  // reload of the Workbench URL (ProjectSetupCard reads this query flag).
  return `http://${formatHostForUrl(host)}:${port}/?session=${encodeURIComponent(token)}&view=workbench`;
}
