...
export function registerArtifactTools(
  ...
  mcp.registerTool("record_artifact_written", {
    description: "Declare a source artifact immediately after writing it. For every Active component implementation, include componentPreview with the exact current runId, ready surfaceId, and entryId returned by get_prototype_rebuild_context.component_preview_targets; never substitute an inventory id. modulePath must equal path. Include exportName, defaultArgs, semanticImpact, and only stateArgs whose keys appear in that target's allowedStateNames; omit stateArgs for undeclared alternative examples. Declare semanticImpact=none only after comparing the implementation with the current component contract and finding no reusable semantic change; use possible when judgment is needed and include semanticEvidenceRecordIds. Runtime detects Next App Router or Vite React, establishes codeLinks, one shared Storybook-free Preview adapter, verified live availability, digest-cached background verification, and internal Verified Candidate automatically. Stop unless the result contains one component Preview exception packet for resolve_component_preview_exception. Never guess identity or author a Story/per-component harness. Rule artifacts retain existing review gates.",
