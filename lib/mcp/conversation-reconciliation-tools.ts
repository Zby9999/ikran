...
export function registerConversationReconciliationTools(
  ...
  mcp.registerTool(
    ...
    {
      description:
        "Complete one bounded conversation review before Rule Update. Call only after the designer explicitly finishes/confirms the design or asks for Rule Update — never during active design turns. Supply the complete ordered Agent-host transcript snapshot between startMessageId and endMessageId plus a structured decision ledger. Every decision must cite at least one designer-authored source message; later explicit designer corrections supersede earlier decisions, while Agent suggestions are never facts by themselves. Runtime atomically stores the immutable transcript snapshot and all derived feedback. Reuse reviewId after interruption: identical replay is idempotent and changed content is rejected. Then pass the returned reconciliation.id to claim_consolidate_review.",
