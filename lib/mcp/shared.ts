...
// source_contract rides claim_initial_design_system_preparation, live-hero
// harness rules ride scaffold_component_harness as live_hero_contract, and
// per-tool semantics live on each tool's description. Never restate contract
// content here. Claude Code truncates this string at 2048 UTF-8 bytes.
export const CLAUDE_MCP_TEXT_BUDGET = 2048;

export const IKRAN_MCP_INSTRUCTIONS =
  "Ikran; open_workbench returns a localhost URL. OPEN-AND-WAIT: open that URL first; while a response re-arms Alignment or Rule Update Review, keep calling wait_for_agent_command and claim each scoped command. All source-of-truth changes go through Ikran tools.\n\n" +
  "DISCIPLINES:\n" +
  "- Read phase + seed count via get_project_readiness; record_new_design_run needs a Seed Reference — else only seed reconstruction.\n" +
  ...
  "- Follow rule taxonomy; route misplaced rules through rule-update proposal, never move silently. Design gen uses design-system source only, never feedback. At completion/Rule Update freeze the transcript; call reconcile_designer_conversation.\n" +
  "- Runtime owns prototype dev servers: declare code, then record_preview; never run one.\n" +
  "- Pass the completed reconciliation to claim_consolidate_review; draft the complete Rule Update Review, publish once, then claim designer decisions. Never write before an accepted decision is claimed.\n" +
  ...
  "On demand: claim_alignment_preparation → Alignment section_contract; claim_initial_design_system_preparation → extraction source_contract. confirm_draft_design_system → get_prototype_rebuild_context → rebuild seed → record_preview. confirm_prototype → reconcile_designer_conversation → claim_consolidate_review → backfill_component_code_links → declare_component_live_heroes → verify_component_live_heroes → formalize_design_system.";
