...
export const IKRAN_MCP_INSTRUCTIONS =
  ...
  "- Follow the source_contract rule taxonomy and self-audit each rule file; propose misplaced-rule moves through the rule-update proposal channel, never move silently.\n\n" +
  "FLOW CONTRACTS are delivered on demand — follow the returned contract, not memory: claim_alignment_preparation returns the Alignment section_contract (card kinds and ordering, title format, evidence target modes); claim_initial_design_system_preparation returns the extraction source_contract (entry kinds, file ownership, token domains, writing styles). Per-tool semantics live on each tool's own description.";
