...
export const CLAUDE_MCP_INSTRUCTIONS =
  "Ikran; open_workbench returns a localhost URL. OPEN-AND-WAIT: open that URL first; during Alignment or Rule Update Review keep calling wait_for_agent_command and claim scoped commands. Source-of-truth changes use Ikran tools.\n\n" +
  "DISCIPLINES:\n" +
  "- Check phase/seeds with get_project_readiness; record_new_design_run needs a Seed Reference, else only seed reconstruction.\n" +
  ...
  "- Declare host edits via record_artifact_written; undeclared files stay out of export_research.\n" +
  "- Never silently drop confirmed input: map it to a record/entry or conflict/omitted/gap. Never invent semantics or use one edited card to formalize unrelated claims.\n" +
  ...
  "- Send reconciliation to claim_consolidate_review; publish review once, then claim decisions. Never write before an accepted decision.\n" +
  ...
  "On demand: claim_alignment_preparation → Alignment section_contract; claim_initial_design_system_preparation → extraction source_contract. confirm_draft_design_system → get_prototype_rebuild_context → rebuild seed → record_preview. Declare exact component provenance plus evidence-backed semanticImpact in record_artifact_written.componentPreview; Runtime links/previews/caches and runs automatic verification. Only resolve_component_preview_exception needs Agent judgment. confirm_prototype → reconcile_designer_conversation → claim_consolidate_review → automatic verification → formalize_design_system.";
