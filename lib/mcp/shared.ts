...
export const IKRAN_MCP_INSTRUCTIONS =
  ...
  "- Follow source_contract rule taxonomy; route misplaced-rule moves via the rule-update proposal channel, never move silently. Design gen uses design-system source only, never feedback; Formalized over Candidate—pass usedCandidateIds; declare conclusions via record_designer_feedback.\n" +
  "- Runtime owns prototype dev servers: declare code, then record_preview; never run one.\n" +
  "- Read feedback only via claim_consolidate_review; narrate only global-rule proposals. Never write a rule-update artifact before confirm_rule_update; declare with that proposalId.\n" +
  ...
  "FLOW CONTRACTS on demand: claim_alignment_preparation → Alignment section_contract; claim_initial_design_system_preparation → extraction source_contract. confirm_draft_design_system → get_prototype_rebuild_context → rebuild seed page from live Figma context, then record_preview. confirm_prototype → claim_consolidate_review → backfill_component_code_links → formalize_design_system(modificationReview) autonomously → record_new_design_run, new host chat.";
