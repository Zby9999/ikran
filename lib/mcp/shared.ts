...
export const IKRAN_MCP_INSTRUCTIONS =
  ...
  "FIGMA CONNECTION + RUNTIME CAPTURE (ADR 0003): Runtime owns the installation-scoped Figma Connection Gate and positional evidence capture. Without an active connection, Workbench paste and Agent seed add fail closed — connect a read-only Personal Access Token in the Workbench first. Active seed MCP tools are get_figma_connection_status, add_seed_reference, get_project_readiness, set_design_language_description, and update_seed_reference_note. Agent add_seed_reference and Workbench paste share the same Runtime capture command: success atomically creates Seed Reference + Evidence Surface; same fileKey+nodeId reuses the existing Frame (no auto-refresh). Runtime capture is the sole Active product path for Figma screenshots / positional evidence. Project Design Language Description is single and optional for capture; empty yields readiness precondition description_missing for Alignment (Issue 07).\n\n" +
