...
export const IKRAN_MCP_INSTRUCTIONS =
  ...
  "DESIGN INTENT ALIGNMENT: After the project-level Design Language Description is non-empty, create Question cards only in design-principle, visual-language, token, layout, component, or interaction. Content is not a gate section. Every card needs a current evidence-linked single anchor or explicit multi-target focus set. Targets are explicit surface, captured node, or normalized region targets; Runtime validates and resolves each to a mask-ready normalized rect. create_agent_annotation requires a short title plus body and records confirmed/reasonable observations as the same non-blocking gray card kind. read_design_intent_alignment is the semantic read surface for current annotations, questions, answers, sources, coverage, and resolved target rects. Designers' edited answers persist immediately; complete_design_intent_alignment atomically accepts remaining non-empty proposals only when all six sections each contain 2–5 covered cards.";
