...
export default defineConfig({
  ...
  // Parallel e2e: each worker spawns its own isolated Ikran Runtime (Next dev
  // on a unique port + own IKRAN_STATE_DIR + own IKRAN_NEXT_DIST_DIR) via the
  // worker-scoped `runtime` fixture in tests/fixtures.ts. No shared webServer —
  // the runtime is a single-project-per-process singleton by PRD design, so
  // test isolation is achieved by giving each worker its own runtime process,
  // not by serializing.
  timeout: 30_000,
  expect: {
    timeout: 10_000
